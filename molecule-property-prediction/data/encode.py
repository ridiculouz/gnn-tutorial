...
class MolGraph:
    def __init__(self, af, bf, us, vs):
        self.atom_features = af
        self.bond_features = bf
        self.start_indices = us
        self.end_indices = vs


def get_graph_from_mol(mol):
    af = np.stack([atom_features(a) for i, a in enumerate(mol.GetAtoms())])
    bf = np.stack([bond_features(b) for b in mol.GetBonds()]) \
        if len(mol.GetBonds()) else np.zeros(shape=[0, num_bond_features()], dtype=np.int)
